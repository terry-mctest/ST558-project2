...
The `carbon` function also converts the bookend timestamps (which are actually textstrings in "YYYY-MM-DDTHH:MMZ" format as required to access the CI API) to simple quasi-date numeric values (e.g. YYYYMM) to facilitate the forthcoming exploratory data analysis (though a proper date value may be required for more rigorous analyses).
